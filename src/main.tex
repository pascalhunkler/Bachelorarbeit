...
\begin{titlepage}
    ...
    {\Huge \theauthor}\\
    \Large{
    Matrikelnummer: 4734792\\
    E-Mail: pascal-hunkler@web.de \\[1cm]
    }
    {\Large Bearbeitungszeit: 27.\,06.\,2022 -- 27.\,09.\,2022} \\[1cm]
...
\begin{acronym}
    ...
    \acro{cfl-bedingung}[CFL-Bedingung]{Courant–Friedrichs–Lewy-Bedingung}
\end{acronym}
...
von Monaghan \cite{monaghan_smoothed_2005}, von Price \cite{price_smoothed_2012} und von Koschier et al. \cite{koschier_smoothed_2020}.
...
Dieser Unterabschnitt basiert auf die Arbeit von Koschier et al. \cite{koschier_smoothed_2020}.
...
\begin{equation}
    -\frac{1}{\rho_f} \nabla p_f \approx -\frac{1}{\rho_f} \sum_{f_f} p_{f_f} \frac{m_{f_f}}{\rho_{f_f}} \nabla W_{ff_f}
\end{equation}
...
Nach Koschier et al. \cite{koschier_smoothed_2020} wird das Euler-Cromer Verfahren bei weitem am häufigsten genutzt.
...
Um einen möglichst großen Zeitschritt zu wählen, der noch zu einer stabilen Simulation führt,
kann sich an der Courant–Friedrichs–Lewy-Bedingung (CFL-Bedingung) orientiert werden.
...
\end{equation}
Die Idee dahinter ist, dass für z.B. $\lambda = 1$ sich die Partikel nicht weiter als eine Partikelgröße bewegen.
In der Literatur werden für $\lambda$ gewöhnlich Werte unter 1 gewählt, wie z.B. $\lambda \approx 0.4$.
...
\begin{align}
    ...
    \textbf{x}_f^{t + \Delta t} &= \textbf{x}_f^t + \Delta t \textbf{v}_f^{t + \Delta t}
\end{align}
Mit diesen Schritten lässt sich ein Simulationsschritt berechnen.
...
\label{chapter:pressure_computation}
...
Anschaulich erklären lässt sich dies anhand eines Beispiels aus der Arbeit von Koschier et al. \cite{koschier_smoothed_2020} mit einer ruhenden Flüssigkeit,
die unter dem Einfluss von Gravitation steht.
...
\end{align}
Dieser Schritt wird solange berechnet, bis ein vorgegebener Dichtefehler unterschritten wird.
Näheres dazu findet sich im Abschnitt \textit{\titleref{sec:pressure_computation}} im folgenden Kapitel.
...
Für die Grenzbehandlung wird der Ansatz aus der Arbeit von Koschier et al. \cite{koschier_smoothed_2020} verwendet,
wo es mehrere Schichten von Grenzpartikel mit identischer Größe und Masse gibt.
...
\begin{equation}
    \rho_f = \sum_{f_j} m_{f_j} W_{f f_j}
\end{equation}
Der Index $f_j$ beschreibt die Nachbarpartikel von $f$, einschließlich der Grenzpartikel.
In der Implementierung ist die Masse aller Partikel identisch, daher kann die Masse der Nachbarpartikel $m_{f_j}$ durch $m_f$ ersetzt werden
und stattdessen die Dichte wie folgt berechnet werden:
\begin{equation}
    \rho_f = m_f \sum_{f_j} W_{f f_j}
\end{equation}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \ForAll{fluid particle f}
            \State $\rho_f \gets m_f \sum_{f_j} W_{f f_j}$
        \EndFor
    \end{algorithmic}
\end{algorithm}

\section{Berechnung des Drucks}
\label{sec:pressure_computation}
...
Im Kapitel \textit{\titleref{chapter:pressure_computation}} wurde zur Bestimmung der Diagonalwerte folgende Gleichung aus \cite{ihmsen_implicit_2014} verwendet,
...
\begin{equation}
    A_{ff} = - \Delta t^2 \frac{m_f^2}{\rho_f^2} \sum_{f_j} \left(\left(\sum_{f_j} \nabla W_{ff_j}\right) + \nabla W_{ff_j}\right) \nabla W_{ff_j}
\end{equation}
...
Ist dieser Fehler unter dem vorgegebenen maximal erlaubten durchschnittlichen Dichtefehler $\eta$,
so wird der letzte berechnete Wert des Druckes genommen, andernfalls wird weiter iteriert und ein neuer Druck berechnet.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \EndFor
         ...
         \While{$e \geq \eta \textbf{ or } iterations < 2$}
            \State $e \gets 0$
            \State $n \gets 0$
...
Berechnet wurden die Simulation auf dem \textit{Intel~Core~i5-6600K} Prozessor mit vier Kernen und Taktfrequenz 4.00~GHz.

Für die IISPH-Simulationen wurde ebenfalls die durchschnittliche Anzahl an Iterationen erfasst,
die während der Druckberechnung benötigt werden, um einen Dichtefehler unter $0.1~\%$ zu ereichen.
Zudem wurde die Zeit gemessen, die für die Druckberechnung und für den Simulationsschritt jeweils insgesamt benötigt wurde.
...
Es wurde eine Steifigkeitskonstante von $k = 7 \cdot 10^5$ gewählt, um den durchschnittlichen Dichtefehler zuverlässig im Bereich unter $0.1~\%$ zu halten.
...
auch wenn die Durchschnittsdichte unter $0.1~\%$ liegt.
...
dass für einen vorgegebenen maximalen Dichtefehler von $0.1~\%$ lediglich der Zeitschritt $\Delta t$ gewählt werden muss.
...
Die Steifigkeitskonstante und der Dichtefehler scheinen damit sich ungefähr antiproportional zueinander zu verhalten.
...
Im Vergleich zur schnellsten IISPH-Simulation ist die SSPH-Simulation somit etwas langsamer, wobei der Unterschied hier weniger stark ausfällt.
...
Nach Koschier et al. \cite{koschier_smoothed_2020} erzielen globale Druckberechnungsmethoden in komplexeren Szenarien,
...
Gründe für diese Beobachtung könnten sein, dass die Simulation noch nicht komplex genug ist,
um einen eindeutigen Performanzgewinn von IISPH im Vergleich zu SSPH zu beobachten.
Auch könnten die Simulationen durch die geringere Geschwindigkeit der Partikel stabiler sein, weswegen der Zeitschreitt bei SSPH höher angesetzt wurde.
...
Im Dammleckszenario mit höherer Flüssigkeitssäule und mehr Partikeln dagegen war die Druckberechnung mit der Zustandsgleichung etwas schneller.
...
Diese Bachelorarbeit beschäftigte sich mit dem Vergleich zweier konkreter Druckberechnungsmethoden.
Es gibt eine zunehmende Anzahl an weiteren Methoden zur Druckberechnung.

Daneben wurde in dieser Arbeit nicht gezeigt, wie sich die Druckberechnung in sehr großen und komplexen Szenarien verhält.

Aus diesen Gründen können noch weitgreifendere Untersuchungen durchgeführt werden, 
in der in möglichst vielfältigen Szenarien globale Drucklöser mit lokalen Drucklösern verglichen werden.
