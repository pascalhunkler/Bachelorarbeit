...
\section{SPH}
Das Konzept \textit{Smoothed Particle Hydrodynamics}
wurde ursprünglich entwickelt, um astrophysikalische Phänomene
besser darstellen zu können
\cite{gingold_smoothed_1977}, \cite{lucy_numerical_1977}.
...
\begin{align}
    \int_{\mathbb{R}^d} W(\textbf{r}', h)dv' = 1\\
    \lim_{h \to 0} W(\textbf{r}, h) = \delta(\textbf{r})
\end{align}
Weitere wünschenswerte Eigenschaften der Kernelfunktion $W$ sind:
\begin{align}
    W(\textbf{r}, h) \geq 0\\
    W(\textbf{r}, h) = W(-\textbf{r}, h)\\
    W(\textbf{r}, h) = 0 \text{ für } \| \textbf{r} \| \geq \hbar
\end{align}
